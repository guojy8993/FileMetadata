...
def check_filename(**metadata):
	...
	""" A special case: "/"  """
	if len(metadata["filename"]) == 0 and ("full_path" not in metadata or not metadata["full_path"] or metadata["full_path"] != "/"):
		raise Exception("API params error: filename can be zero-length String only when full_path is '/' ")
...
class API(object):
	...
	def get_metadata(self,**args):
	  	if "filter" not in args or args["filter"] is None or not isinstance(args["filter"],dict):
                        raise Exception("API params incomplete")
                filter = args["filter"]
                if len(set(filter.keys()) - set(METADATA_PROP_VALIDATOR_MAPPING.keys())) > 1 or len(filter.keys()) <> 4:
                        raise Exception("API params error: there exists non-metadata-property data.")
                check_full_path(**filter)
                check_filename(**filter)
                check_isdir(**filter)

		if "depth" not in filter or filter["depth"] is None or not isinstance(filter["depth"],int):
			raise Exception("API params error: depth (integer value) required.")	        
		depth = filter["depth"]		
		...
		filter_conditions = {}
                if filter["isdir"] == 1:
                        OR_LIST = []
                        """ match all child files existing in this directory """
			""" add a secial case, full_path -> '/' & filename -> '' """
			current_namespace = "%s%s" % (filter["filename"],"" if len(filter["filename"])==0 else os.sep)
			ndepth_match = "%s%s" % (REGEX_DIR_NAME,os.sep)
                        subfiles = "%s%s(%s){0,%d}$" % (filter["full_path"],current_namespace,ndepth_match,depth)
			...
			""" add a special case:full_path -> '/' & filename -> '',skipping matching current directory,
			    only matching subfiles ."""
			if len(filter["filename"]) > 0:
				""" pop non-metadata-property data"""
				del filter["depth"]
                        	OR_LIST.append(filter)
